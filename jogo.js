(() => {
  ...
  const PLAYER_RUN_SPRITE_HEIGHT = 88;
  const EMOTE_DURATION = 3;
  const JOJO_REVEAL_DELAY = 1.25;
  ...
  const richardJojoPointSprite = new Image();
  richardJojoPointSprite.src = 'fotos/richard-jojo-apontando-v2.png';
  let jojoPointFrames = [];

  const richardJojoStandSprite = new Image();
  richardJojoStandSprite.src = 'fotos/richard-jojo-stand-v2.png';
  let jojoStandFrames = [];
  ...
  function endGame(won) {
    ...
    overlayCard.innerHTML = won
      ? `<span class="overlay-icon" aria-hidden="true">★</span>
         ...
         <div class="victory-applause" aria-label="Três personagens comemorando e aplaudindo Richard">
           <span class="victory-applause__person">
             <span class="victory-applause__sprite victory-applause__sprite--conductor" role="img" aria-label="Regente aplaudindo"></span>
           </span>
           <span class="victory-applause__person">
             <span class="victory-applause__sprite victory-applause__sprite--blue" role="img" aria-label="Integrante de azul aplaudindo"></span>
           </span>
           <span class="victory-applause__person">
             <span class="victory-applause__sprite victory-applause__sprite--bowtie" role="img" aria-label="Integrante de gravata aplaudindo"></span>
           </span>
         </div>
  ...
  function startEmote(frame) {
    ...
    player.emoteTimer = frame === 4 ? EMOTE_DURATION + JOJO_REVEAL_DELAY : EMOTE_DURATION;
    player.vx = 0;
  }
  ...
  function update(dt) {
    ...
    if (player.grounded && Math.abs(player.vx) > 18) {
  ...
  function drawPlayer() {
    ...
    let targetHeight = PLAYER_SPRITE_HEIGHT;
    let allowFlip = true;

    if (player.emoteFrame !== null) {
      const isTPose = player.emoteFrame === 3;
      const isJojoPose = player.emoteFrame === 4;
      const jojoStandRevealed = isJojoPose && player.emoteTimer <= EMOTE_DURATION;
      if (isJojoPose) {
        image = jojoStandRevealed ? richardJojoStandSprite : richardJojoPointSprite;
        frames = jojoStandRevealed ? jojoStandFrames : jojoPointFrames;
        frame = 0;
        targetHeight = jojoStandRevealed ? 150 : PLAYER_SPRITE_HEIGHT;
      } else {
      ...
      targetHeight = PLAYER_RUN_SPRITE_HEIGHT;
    }
  ...
  function handleKey(event, pressed) {
    ...
    const controlKeys = ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'Space', 'KeyA', 'KeyD', 'KeyW', 'KeyE', 'Enter', 'Digit1', 'Digit2', 'Digit3', 'Digit4', 'Digit5', 'Numpad1', 'Numpad2', 'Numpad3', 'Numpad4', 'Numpad5'];
  ...
  const prepareRichardJojoPointFrames = () => {
    jojoPointFrames = analyzeSpriteSheet(richardJojoPointSprite, 1);
    if (!running) draw();
  };
  const prepareRichardJojoStandFrames = () => {
    jojoStandFrames = analyzeSpriteSheet(richardJojoStandSprite, 1);
    if (!running) draw();
  };
  ...
  richardJojoPointSprite.addEventListener('load', prepareRichardJojoPointFrames);
  richardJojoStandSprite.addEventListener('load', prepareRichardJojoStandFrames);
  ...
  if (richardJojoPointSprite.complete && richardJojoPointSprite.naturalWidth) prepareRichardJojoPointFrames();
  if (richardJojoStandSprite.complete && richardJojoStandSprite.naturalWidth) prepareRichardJojoStandFrames();
})();
